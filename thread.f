...
c      del = 1.d0
      del2 = del*del
      FVAL(1) = -0.5d0*U(1)*UX(2)-U(2)*UX(1)
      FVAL(2) = -U(2)*UX(2)+ C*(3.d0*UXX(2)+6.d0*UX(1)*UX(2)/U(1)
     +  + UX(1)/U(1)**2+del2*UX(3) -2.d0*A1*U(2)/U(1)**2)
      FVAL(3) = (UXX(1) - U(3))/eps
C
      RETURN
      END
C-----------------------------------------------------------------------
...
C
      ...
      DFDU(1,2) = -U(1)
      DFDU(1,3) = 0.0
      DFDU(2,1) = -6.0*C*UX(1)*UX(2)/U(1)**2
     +	-2.0*UX(1)/U(1)**3 + 4.0*A1*U(2)/U(1)**3
      DFDU(2,2) = -UX(2)-2.0*A1/U(1)**2
      DFDU(2,3) = 0.0
      DFDU(3,1) = 0.0
      DFDU(3,2) = 0.0
      DFDU(3,3) = -1.0/eps
      ...
      DFDUX(1,2) = -0.5*U(1)
      DFDUX(1,3) = 0.0
      DFDUX(2,1) = 6.0*C*UX(2)/U(1)+1.0/U(1)**2
      DFDUX(2,2) = -U(2)+6.0*C*UX(1)/U(1)
      DFDUX(2,3) = del2
      DFDUX(3,1) = 0.0
      DFDUX(3,2) = 0.0
      DFDUX(3,3) = 0.0
...
c      a = 1.0d0
c      aa1 = 0.1*a
      U(1) = 1.0+0.5*dcos(w*X)
      U(2) = 0.0
      U(3) = -0.5*w*w*dcos(w*X) 
...
c-----------------------------------------------------------------------
c
      write(nout,95) 'The Thin thread system'
