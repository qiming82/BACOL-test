...
c
c In order to run an example, this source code must be linked with
c three other compilation units:
c 1. The supporting routines collected in bacoli-aux.f.
c 2. The supporting routines in d1mach_i1mach.f.
c 3. A driver program, e.g., driver-curve.f.
c 4. PDE system definition subroutines, e.g., rcdsys.f.
c Note that the number of PDES, npde, specified in 3. and 4. must agree.
